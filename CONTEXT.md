...
A Tauri desktop app with an embedded AI pet (Cloudia) that surfaces notifications via bubble templates, corner toasts, and an interactive panel. Multi-vault, multi-feature (analyze / clips / schedule / wiki), plugin-host AI capability for trusted-tier plugins.
...
_Avoid_: Template Agent, Feature Agent (overlaps with `runFeatureAgent`-backed feature agents like wiki/analyze), Plugin Agent (overlaps with `buildPluginAi.agent`).

**Feature Agent**:
A vault-scoped agent loop backed by `runFeatureAgent` — spawns the `claude-cli` sidecar with tools, file-system access, and a canonical `agents/<feature>.md` system prompt seeded into `<vault>/__<feature>__/.claude/`. Currently registered: analyze, clips, schedule, wiki. Distinct from the Bubble Template AI Agent despite both being called "agent" in casual speech.

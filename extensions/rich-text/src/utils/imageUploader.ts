// ponytail: local-only copy of the host's utils/imageUploader.ts. The host
// version ships four strategies (local/r2/qiniu/oss); the latter three pull
// in host storage stores (storageConfigStore, registry, types) the extension
// can't reach. The rich-text extension only exercises 'local' (ImagePasteDialog
// is local-only, RichTextEditor's handleImageConfirm special-cases local).
// LocalFileStrategy + the conversion helpers are copied verbatim; the
// storage-provider strategies are dropped. Same reason as ImagePasteDialog.

/** Upload target types */
export type UploadTarget = 'local';

// ponytail: local-only copy of the host's ImagePasteDialog. The host version
// supports four upload targets (local/r2/qiniu/oss) via IconSelect +
// ThemeIcon — both pull in host stores (storageConfigStore, appearanceStore)
// the extension can't reach. The rich-text extension only exercises the
// 'local' target (RichTextEditor's handleImageConfirm special-cases local;
// other targets fall through to result.markdownUrl but the SDK exposes no
// storage-provider config). So this copy ships local-only: same form
// (fileName / directory / format / size / path preview), no target picker.
// Mirrors the host overlay style so the dialog looks identical.
...
export function ImagePasteDialog({
  ...
  }, [fileName, format, directory, width, height, originalWidth, originalHeight, onConfirm]);
    ...
    <div className="img-paste-overlay fixed inset-0 z-[200] bg-black/45 flex items-center justify-center animate-[fadeIn_.15s]" onKeyDown={handleKeyDown}>
      <div className="img-paste-dialog bg-panel border border-brd2 rounded-xl shadow-[0_16px_48px_rgba(0,0,0,.2)] w-[520px] max-w-[92vw] animate-[slideUp_.2s] overflow-hidden">
        ...
        <div className="py-1.5 px-[18px]">
          ...
          <div className="flex items-center gap-2 py-[7px] px-2.5 border border-brd2 rounded-md bg-surf text-t2 text-[13px]">
            <FolderOpen size={14} />
            <span>{t('editor:imagePaste.targets.local')}</span>
          </div>
        </div>
        ...
        <div className="py-1.5 px-[18px]">
          ...
          <input
            ...
            onCompositionEnd={(event) => {
              ...
              setFileName(event.currentTarget.value.replace(/[^\w一-鿿㐀-䶿豈-﫿-]/g, ''));
            }}
            onChange={(event) => {
              if (isComposingRef.current) {
                setFileName(event.target.value);
              } else {
                setFileName(event.target.value.replace(/[^\w一-鿿㐀-䶿豈-﫿-]/g, ''));
              }
            }}

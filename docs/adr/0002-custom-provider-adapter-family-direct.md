...
- **`ModelServicesSettings` no longer shows an API path preview for custom providers.** The preview used `getEndpointPath(entry.defaultChatEndpoint)`, keyed by endpoint name. `adapterFamily` is a bundled id, not an endpoint key, so `getEndpointPath` would return null anyway. Pre-launch acceptable; a future task can derive the path from `baseUrl + adapterFamily → path-suffix` if the preview is wanted back.

...
export const exportEnhancers = {
  'enhance-quote': async (body) => { body.classList.add('exported'); },
};
...
await ctx.ai.chat({ sessionId: 'my-plugin', prompt: 'Summarize', onEvent: (e) => {} });
await ctx.ai.agent({ feature: 'study', instruction: 'Review', onEvent: (e) => {} });
await ctx.ai.editFile({ path: 'notes/summary.md', instruction: 'Summarize as 3 bullets', onEvent: (e) => {} });
await ctx.ai.createFile({ path: 'notes/new.md', instruction: 'Draft skeleton', onEvent: (e) => {} });
```

...
<h1 align="center">Folyn エディター</h1>
...
- **拡張機能システム** — マイクロカーネル + 拡張機能 SDK アーキテクチャ。翻訳・スケジュール・Wiki は拡張機能として提供され、サードパーティ拡張も可能。
...
Office ドキュメント、音声・動画、アーカイブ、電子書籍、プレゼンや図面を Folyn から離れずに表示——別途ソフトをインストールする必要はありません。
...
Folyn ワークスペース内にターミナルパネルを開き、エディタと並列表示、ファイルパスは自動同期。Claude Code / Codex CLI などのエージェントが現在のドキュメントを直接変更でき、AI の編集結果は即座にエディタに反映されます。コマンド出力はカーソル位置に貼り戻せ、「編集 → 呼び出し → 貼り戻し」を一条のパイプラインに圧縮します。

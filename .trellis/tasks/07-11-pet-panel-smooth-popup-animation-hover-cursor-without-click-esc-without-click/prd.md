...
- macOS-only (the NSPanel backend is macOS; `QUILL_PET_PANEL_BACKEND=legacy` fallback exists but is not the focus).
...
2. Hovering the pet mascot shows the hand cursor on first hover (no click needed), even when Quill is not the frontmost app.
   - **Use the `tauri-nspanel` crate's native `tracking_area`** (`ActiveAlways | CursorUpdate | MouseEnteredAndExited | InVisibleRect`) on `QuillPetPanel` + a `panel_event!` `on_cursor_update`/`on_mouse_exited` handler that sets the NSCursor. `ActiveAlways` is the flag that delivers hover events when the app isn't frontmost. No raw objc.
...
- [ ] First hover over the pet mascot (with Quill NOT frontmost) shows the hand cursor; moving the cursor away restores the default cursor — no click required.
...
- Add `with: { tracking_area: { options: TrackingAreaOptions::new().active_always().cursor_update().mouse_entered_and_exited().in_visible_rect(), auto_resize: true } }` to the `QuillPetPanel` `panel!` macro in `pet_panel_macos.rs`.

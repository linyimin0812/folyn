...
`owner = the panel` (`RawQuillPetPanel`, which already overrides
...
`NSWindow` / our `RawQuillPetPanel`); see `pet_make_transparent`
...
**Option (d): a manually-added `NSTrackingArea` with `owner = RawQuillPetPanel`
...
- Smallest diff: remove the `with: { tracking_area }` block from the
  `panel!(QuillPetPanel { ... })` macro, and in `convert_windows` (after
...
| `apps/desktop/src-tauri/src/pet_panel_macos.rs` | the `panel!(QuillPetPanel { ... with: tracking_area ... })` config + the `on_cursor_update`/`on_mouse_exited` handler wiring in `convert_windows` |

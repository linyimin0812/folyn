...
objc. The crate v2.1 (already a Folyn dependency) ships first-class
`tracking_area` + `cursorUpdate` via the `panel!` / `panel_event!` macros.

## How it works

1. Add a `with: { tracking_area: { options, auto_resize } }` block to the
   existing `FolynPetPanel` `panel!` macro in `pet_panel_macos.rs`.
   ```rust
...
- `NSTrackingActiveAlways` makes the tracking area deliver mouse/cursor events
  ...
  flag that solves "hand cursor on plain hover when Folyn isn't frontmost."
...
- `pet_panel_macos.rs`: add `with: { tracking_area: {...} }` to `FolynPetPanel`
  `panel!`; define + wire a `panel_event!` handler with `on_cursor_update` /
  `on_mouse_exited`.
- `convert_windows`: attach the handler to the `FolynPetPanel` panel instance.

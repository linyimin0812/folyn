...
**注意**：cpal 在 Linux 用 ALSA（需 `libasound2-dev`），Folyn 不支持 Linux，所以迁到 `[dependencies]` 会拉 ALSA 编译，但 Linux target 不在 CI matrix，**只要 `cargo check --target x86_64-pc-windows-msvc` 和 macOS 都通过即可**。更稳的做法：

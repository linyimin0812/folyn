...
candle 阶段已通过，问题在 light 链接 MSI。继续走 MSI 路线要么加 `--verbose` 反复跑 CI 抓错，要么绕开 WiX。Folyn 无企业批量部署需求，不需要 MSI。
...
- CI 触发 release workflow 后 Windows job 应产出 `Folyn_0.1.0_x64-setup.exe` 而非 `.msi`。

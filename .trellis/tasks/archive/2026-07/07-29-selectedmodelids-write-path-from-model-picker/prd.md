...
When the user selects a model in the "获取模型" picker, persist the selection to `~/.folyn/providers/settings.json`'s `selectedModelIds` field. Currently the picker's `onSelect` only writes `chatModel` (the active model); `selectedModelIds` is never written, so it stays `[]` forever.
...
- For each id, display model details (display name, group, capabilities) fetched from `~/.folyn/providers/{chatProvider}/models.json` via `readUserProviderModels(chatProvider)`.
...
- [ ] After picking a model in the picker, `~/.folyn/providers/settings.json` has that id in `selectedModelIds[<providerId>]`.
...
- [ ] Each row displays model details from `~/.folyn/providers/{chatProvider}/models.json` when present; falls back to id-only when the file or id is missing.
...
- Reconciling `~/.folyn/providers/{p}/models.json` capability taxonomy with the bundled catalog (separate task per `userProvidersCatalog.ts:13-16`).
...
### Bug: migration re-runs every boot and clobbers `~/.folyn/providers/settings.json`

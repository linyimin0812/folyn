...
- **Query**: Which evidence-backed learning methods exist, how do mainstream tools implement them, and how to embed each cheaply into Folyn's markdown-driven "Study" workbench (one .md per topic, `## 资料`/`## 计划`/`## 笔记` sections, AI via CLI adapter, existing Pomodoro + schedule + wiki links + callouts).
- **Scope**: mixed (external learning science + internal repo grounding)
- **Date**: 2026-06-29

> Note: the `mcp__exa__*` search tools were not available in this research environment. Findings below draw on established, well-documented learning-science literature and the canonical SM-2 spec, cross-checked against the actual Folyn repo patterns (cited with file:line). Treat the tool-UX descriptions as widely-known public behavior, not freshly-verified citations.
...
## 2. Methods → tool reference → Folyn embedding → MVP-fit

| Method | Core idea (one line) | Why it works | What learner does | Tool impl (UX) | Folyn embedding (concrete) | MVP-fit |
|---|---|---|---|---|---|---|

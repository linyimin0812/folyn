# Research: Tauri 2 Runtime Plugin Loading for folyn
...
- **Date**: 2026-07-08
- **Verified against**: vendored Tauri `2.11.2` at `~/.cargo/registry/src/index.crates.io-1949cf8c6b5b557f/tauri-2.11.2/` and folyn's own `apps/desktop/src-tauri/`.

---

## TL;DR — Realistic MVP path

**Simplest way to get a third-party JS plugin running in folyn today with a sane security boundary:**

1. Plugin = a folder under `~/.folyn/plugins/<id>/` containing `manifest.json` + `index.js` (UMD/IIFE bundle exposing a global or a `default export`-shaped factory) + optional `index.html`.
2. Rust side serves the plugin folder over a **custom URI scheme** registered via `tauri::Builder::register_uri_scheme_protocol("folyn-plugin", ...)` (verified: `tauri-2.11.2/src/app.rs:2122`). The handler reads from disk and returns bytes with correct `Content-Type`.
3. Plugin UI runs inside a **sandboxed `<iframe>`** (`sandbox="allow-scripts"`, NO `allow-same-origin`) loaded from `folyn-plugin://localhost/<id>/index.html`. Host communicates via `postMessage`. This gives origin isolation by default.
...
- `asset://` protocol (the default `convertFileSrc` scheme) is scoped to `$HOME/**` + `$APPDATA/**` in folyn today (`tauri.conf.json:79-82`). A plugin under `~/.folyn/plugins/` is reachable via `convertFileSrc` already — but the asset protocol's `Content-Type` may not be `text/javascript`, so `import()`-ing it can fail on strict MIME checks. A custom scheme handler (§2) is the robust path because you control the headers.
- folyn currently sets `app.security.csp = null` (`tauri.conf.json:76`). Any plugin-loading design should add a CSP that forbids remote script sources; otherwise a loaded plugin can pull arbitrary remote code.
...
- **Origin.** folyn's production webview origin is `tauri://localhost` on macOS/Linux, `http://tauri.localhost` on Windows (Tauri docs + `tauri-2.11.2/src/app.rs:2118-2119`). A dynamic `import()` of a URL whose origin differs from the webview origin is subject to the browser's same-origin/CORS rules for module scripts. Module scripts are fetched with `Sec-Fetch-Mode: cors` and require CORS headers on the response. So:
...
**Recommendation for folyn MVP:** require plugins to be **self-contained ESM bundles** (no further `import` of remote URLs) and load them via the **custom-scheme handler** in §2 (same-origin, no CORS, correct MIME). Reserve `import()` of remote CDN URLs for the "trusted first-party" tier only.
...
#### 2a. `asset://` protocol (built-in, already enabled in folyn)

- Folyn's `tauri.conf.json:77-83` enables `assetProtocol` with scope `$HOME/**`, `$APPDATA/**`.
...
- A plugin folder at `~/.folyn/plugins/<id>/index.js` IS under `$HOME/**`, so `convertFileSrc("/Users/.../.folyn/plugins/<id>/index.js")` returns a URL the webview can `fetch()`.
...
- The handler receives an `http::Request<Vec<u8>>` and returns `http::Response<T>`. You control status, headers (incl. `Content-Type`, `Content-Security-Policy`), and body. So you can:
  - Serve `~/.folyn/plugins/<id>/index.js` with `Content-Type: text/javascript`.
...
- Origin behavior (verified `tauri-2.11.2/src/app.rs:2113-2120`): a page loaded from a custom scheme has origin `<scheme>://localhost` on macOS/Linux, `http://<scheme>.localhost` on Windows. So a plugin page served from `folyn-plugin://localhost/<id>/index.html` has a **different origin** from the main app's `tauri://localhost` — giving you origin-level isolation for free. The host can still talk to the plugin via `postMessage` (cross-origin) or via a `window.postMessage` bridge.
- This is the **same pattern** folyn already uses for embedded webviews (`apps/desktop/src-tauri/src/commands.rs:91` `create_webview`), except those load external URLs. Loading from a custom scheme is strictly simpler.

**Feasibility verdict: WORKS, and is the recommended MVP mechanism.** No repackage needed; you add one Rust `register_uri_scheme_protocol` call and serve files from `~/.folyn/plugins/`.
...
**Recommendation:** MVP = **sandboxed iframe** for inline UI plugins + **separate WebviewWindow** for "tool window" plugins (the uTool window pattern). Both load from the custom `folyn-plugin://` scheme so they get cross-origin isolation from the host automatically.
...
let cap = CapabilityBuilder::new(format!("plugin-{id}"))
    ...
    .permission_scoped(
        ...
        vec![ScopeValue::Path(PathBuf::from(format!("~/.folyn/plugins/{id}/data")))],
        vec![],
    );
...
- Signing: **ed25519** over the SHA-256 of the canonicalized manifest + file list. Crates:
  - `ed25519-dalek` (pure Rust, widely used) — sign with publisher private key, verify with public key pinned in folyn's config.
  - `sha2` for hashing.
...
- **Webview destroy/recreate (for tool-window plugins):** `WebviewWindow::destroy()` then rebuild. Clean. Folyn already has `close_webview` / `hide_webview` commands (`commands.rs` invoke_handler list in `lib.rs:289-295`).
...
1. **Plugin shape.** `~/.folyn/plugins/<id>/{manifest.json, index.html, index.js, style.css}`. `manifest.json` declares `id`, `version`, `publisher`, `permissions` (host-mediated, NOT raw Tauri perms in MVP), `contributionPoints` (e.g. `{ "command": { "id": "...", "title": "..." } }`).
2. **Rust: custom scheme.** In `apps/desktop/src-tauri/src/lib.rs` `run()`, before `.build()`:
   ```rust
   .register_uri_scheme_protocol("folyn-plugin", |ctx, request| {
       // parse plugin id + path from request.uri()
       // read file from ~/.folyn/plugins/<id>/<path>
       // return bytes with correct Content-Type + a per-plugin CSP header
   })
   ```
3. **Rust: install command.** A new `#[tauri::command] install_plugin(id, source_path)` that: copies/extracts the bundle to `~/.folyn/plugins/<id>/`, verifies signature (§5), writes a record to a `plugins.json` registry, and emits a `plugin://installed` event to the main window.
4. **React host.** `PluginHost` service that, on `plugin://installed`, creates a hidden `<iframe sandbox="allow-scripts" src="folyn-plugin://localhost/<id>/index.html">`, wires `postMessage` to a vetted RPC bridge (fs read limited to plugin's data dir, http fetch with allowlist, etc.). The plugin's contributed commands appear in the command palette via `commandRegistry.register`.
...
| `apps/desktop/src-tauri/src/commands.rs:91` (`create_webview`) | Folyn already creates embedded webviews with `initialization_script` — proof the per-plugin-webview path works |

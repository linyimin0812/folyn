...
让 `create-folyn-plugin` 模板的 `build.mjs` 产出一个**可直接选目录安装**的结果目录——`dist/` 内含 `manifest.json` + built bundle（+ 静态资源），用户在 Plugins 设置里选 `dist/` 文件夹就能装，不混入源码 / 配置 / node_modules。
...
- 模板路径：`packages/create-folyn-plugin/template/`。
...
- [ ] `create-folyn-plugin` 脚手架后 `pnpm install && pnpm build` 即可产出可装目录。

## Definition of Done

- 单测/集成验证：脚手架一个临时插件 → `pnpm build` → 检查 `dist/` 内容 + manifest `main` 字段。
- `pnpm --filter create-folyn-plugin test`（若脚手架有 test harness）绿。
- README 段落更新。
...
- 改 `create-folyn-plugin` CLI 本身（只改 `template/`）。

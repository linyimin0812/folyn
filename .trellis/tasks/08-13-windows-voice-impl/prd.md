...
- [ ] 验证 `voice.rs` 的 `paste_log` 在 Windows 上落位 `%LOCALAPPDATA%\quill\logs\quill-voice-debug.log`（`dirs::cache_dir()` Windows 返回 `%LOCALAPPDATA%`）。如果 `dirs::cache_dir()` 返回 None 或路径不符，加 `dirs::cache_dir().or_else(|| dirs::data_dir())` fallback。
...
- SendInput 前台窗口竞争：Quill 必须 `window.hide()` pre-paste（镜像 macOS `insertion.rs:680-699` hide-orb 模式，research summary 风险点 5）

...
Enable Folyn plugins to contribute **their own full-page UI** (like uTools' full-window tools), so plugin authors can build self-contained mini-apps (image viewer, color picker, clipboard manager, etc.) inside Folyn — not just markdown directives or commands.
...
- Trusted tier loads via blob-URL `import()` (`trustedLoader.ts:90-92`); sandbox tier uses `sandboxLoader.ts` (iframe + postMessage RPC via `folyn-plugin://localhost` origin).
...
- Rust-side `folyn-plugin://` URI scheme handler exists in `apps/desktop/src-tauri/src/plugin_commands.rs` — serves static HTML/JS/CSS from `~/.folyn/plugins/<id>/`. Path-traversal hardened. **No `/rpc` POST endpoint yet** — this task adds it.
...
- Opening creates a Tauri `WebviewWindow` whose URL is `folyn-plugin://localhost/<plugin-id>/<entry>`.
- The `folyn-plugin://` URI handler gains a POST `/rpc` route: plugin JS calls `fetch('folyn-plugin://localhost/<id>/rpc', { method: 'POST', body: { method, params } })`; handler validates against `PluginPermissions` (reuses `rpcBridge.ts` `isPathInScope` + permission checks) and dispatches to a host-side method table.
...
- [ ] Plugin JS `fetch('folyn-plugin://localhost/<id>/rpc', ...)` reaches the host and returns a JSON result.
...
- `sandboxLoader.ts` uses `folyn-plugin://localhost/<id>/<html>` custom-scheme URL for iframe src. Same URL scheme can serve a WebviewWindow's `url:` option — no new serving path needed.

...
- **Scope**: mixed (external scheme survey + internal Folyn mapping)
- **Date**: 2026-07-05

## TL;DR Recommendation for Folyn
...
**Recommended Folyn model** (3-tier, Cryptomator-style):
...
**Recommendation for Folyn**: PBKDF2-SHA256, 600k iterations, 16-byte salt. All in renderer via `crypto.subtle.deriveKey`. ~1-2s on a typical laptop at 600k — acceptable for a once-per-session unlock. If unacceptable, move Argon2id to Rust as a fast tracked upgrade (Argon2id t=3, m=64MiB is also ~1s and is the modern preferred KDF).
...
- Vault-level remote object `__folyn__/keywrap.json`: `{version, kdf:{algo,iter,salt}, wrappedMasterKey: <base64>, nonce}`.
...
For Folyn (server = dumb S3/WebDAV, no auth backend), the **Cryptomator model fits best**:
...
1. First device: user enables E2E in Settings, sets passphrase.
   ...
   - Upload `__folyn__/keywrap.json = {version, kdf:{...}, wrappedMaster, nonce1}`.
...
| Vault key manager (load/save keywrap, unlock) | `apps/desktop/src/services/sync/cryptoKeyManager.ts` | Talks to `S3VaultProvider` to fetch/push `__folyn__/keywrap.json`. Caches unwrapped master key in-memory for session. |

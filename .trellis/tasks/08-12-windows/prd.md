...
让 quill 桌面端（Tauri + React/TS）在 Windows 11 上能正常构建、启动、运行所有核心功能；盘点所有 macOS 专属依赖并修复，包括 pet/voice 模块的 Windows 原生重写与 Chrome 密码导入的 DPAPI 实现。
...
- [ ] R11 `src-tauri/src/voice.rs:33-34`：paste_log 路径改 `dirs::cache_dir().join("quill/logs")`

...
> 来源：trellis-research 子代理（2026-07-06），读取 GitHub `ahkohd/tauri-nspanel` v2.1 源码 + BongoCat 实际用法 + Mochi git 历史。
...
## 3. 安全性 vs Mochi 的 crash

Mochi `c2269ab`（已回退 `81bc9b8`）的失败三要素：
...
- **Mochi 当前**：`move_to_active_space | full_screen_auxiliary | full_screen_allows_tiling` = 2|256|512 = **770**
...
Mochi `267583e` 注释"窗口不在任何 Space"是**特定于 Mochi 当时用 NSWindow（非 NSPanel）+ ScreenSaver level 的误诊**。`CanJoinAllSpaces + FullScreenAuxiliary` 只对 AppKit 当作 panel 的窗口（NSPanel 子类，或 panel/utility level 的窗口）才授予全屏 Space 可见性。普通 NSWindow 在 ScreenSaver level (1000) 太高，AppKit 把它放在全屏 App 遮挡的独立层。**真 NSPanel + Dock level (20) + 273** 才是正确配方。
...
Mochi 当前 `objc` 0.2.7 是 **deprecated 且 unsound**（维护者推荐迁 `objc2`）。

**crate 的核心价值**：sound 宏生成的子类（空 ivar + 鼠标事件重写 = 防 crash 的关键）、可逆 `to_window()`、`ManagerExt`/`WebviewWindowExt` 与 Tauri State 集成、`PanelBuilder` fluent API、`panel_event!` 类型安全 delegate、~10 个生产 App 验证含同用例 BongoCat。DIY 无实质 upside 且正是咬过 Mochi 的风险面。

## 最终结论

1. **`to_panel()` 在 Mochi Tauri 2 webview 窗口上安全**——同样是 `object_setClass`，但 swap 到自定义 `objc2` `RawNsPanel` 子类（空 ivar + 鼠标事件重写），与 Mochi `c2269ab`（base NSPanel + objc 0.2.7 + 无重写）本质不同。在 `setup` hook 主线程、窗口创建后立即调用。
...
4. **"stationary 与 canJoinAllSpaces 冲突" 是误诊**，是 Mochi NSWindow+ScreenSaver 上下文的产物，非 AppKit 通则。

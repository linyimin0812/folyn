...
参考 `/Users/yiminlin/project/llm_wiki` 的 LLM 层设计，重构 quill 的 LLM 部分，消除重复、去掉投机性抽象、统一 prompt 与 provider 扩展点。具体范围待 brainstorm 收敛。
...
2. `@quill/cli-adapter` 包导出（`createAdapter`/`listAdapters`/`ClaudeAdapter`/`buildClaudeArgs`/`buildClaudeShellCommand`/`quoteShellArg`/类型）。
...
6. 磁盘会话 `~/.quill/chat-sessions/<id>.json` 格式 `{role,content}`。
...
* quill 双通路：CLI（`packages/cli-adapter`）+ Rust（`apps/desktop/src-tauri/src/chat.rs` 272 行，rig-core 0.40）。
...
* **与 quill `ClaudeAdapter` 本质区别**：quill 把 CLI 当**带状态的 agent runtime**（`start/send/stop`、`resumeSessionId`、8 类事件含 `tool_start`/`tool_end`/`file_change`、`permissionMode`/`agent`/`agents`/`addDir`/`bare`/`systemPrompt`）；llm_wiki 把 CLI 当**只产文本的 LLM provider**。

### 调研结论

* **值得借鉴**：llm_wiki 的 **`streamChat` 分流模式**——按 `provider` 在入口早返，CLI/HTTP 签名对齐，让上层调用方零分支。
* **不值得借鉴**：quill 的 `tool_use`/`file_change`/`resume`/`permissionMode`/`agent` 编排是交互式 agent 面板的核心价值，不应为了统一接口砍掉。**两套抽象并存合理**。
* **重构真正价值点**：让 quill 的 `CliAdapter`（CLI 通路）与未来的 TS 侧 provider 抽象（HTTP 通路）**在入口分流签名对齐**，而不是强行合一。调 `streamChat` 的业务调用方（如 wikiIngestService/githubAnalysisService 等需要纯文本输出的场景）可以选 CLI 或 HTTP，互为 fallback。

...
`FolynPetPanel` class but fails — the observer was registered against
the original NSWindow class before `to_panel()` swapped it.

## Root cause

`convert_windows()` in `apps/desktop/src-tauri/src/pet_panel_macos.rs`
calls `window.to_panel::<FolynXxxPanel>()` on each Tauri WebviewWindow.
`to_panel()` uses `object_setClass` to swap the window's class to our
custom `objc2`-defined `RawFolynXxxPanel` subclass. macOS's Touch Bar

...
在 Quill 加一个 **存储与分享 settings tab**，配置对象存储凭据后同一份配置服务两个用途：
...
- **Settings 持久化套路**:
  ...
  - 密钥 slice: `aiConfigStore` 走 `~/.quill/providers/<provider>.json`（隔离）—— 存储凭据应跟这条
...
- [ ] 新增 `storage` settings tab + 持久化 slice（凭据存 `~/.quill/image-hosts/<provider>.json`）

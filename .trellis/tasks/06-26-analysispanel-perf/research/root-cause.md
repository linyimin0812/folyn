...
`reports/2026-06-14-folyn.html`. Both `parseHtmlForGrapes(content)` (run
...
```
$ ls -la /Users/yiminlin/folyn/default_vault/reports/
-rw-r--r--  83145 bytes  2026-06-14-drawio-skill.html
-rw-r--r-- 26887926 bytes 2026-06-14-folyn.html        ← 26 MB
-rw-r--r--  60211 bytes  2026-06-14-trellis.html
-rw-r--r--      40 bytes 2026-06-14-folyn.tags.json
```

`reports/2026-06-14-folyn.html` is 26 MB of self-contained HTML (huge inline
...
If `reports/2026-06-14-folyn.html` was previously opened (or restored via
...
start (no pre-open tab), clicking the folyn report card will freeze the app.
...
- **Pros**: Eliminates path-1 freeze (panel switch reactivating the folyn
...
- The task framing assumed `loadReports` was the bottleneck. Code reading
  ...
  reactivating the 26 MB `reports/2026-06-14-folyn.html` tab.
- I did not run the app. Confirmation experiment: with the folyn.html tab
  open, click the 项目分析 activity and time the freeze; then close the
  folyn.html tab and click again — the freeze should disappear. That
  isolates path 1 from path 2.
- I did not inspect whether `restoreOpenTabs` actually reopens folyn.html on

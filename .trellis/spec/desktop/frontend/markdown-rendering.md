...
The same rule also hints the rasterizer toward geometric precision (`shape-rendering: geometricPrecision` on `mjx-container svg`, `text-rendering: geometricPrecision` on `mjx-container`) for 1x DPI (non-Retina) screens. With the font pin alone, the SVG viewBox (~814×1058 internal units) is rasterized to ~13×17 device pixels on a 1x screen — a 60x downsample that reads as blurry subpixel anti-aliasing. The hint is a ponytail rung one CSS rule, no layout side effects below the 2x-render-and-scale upgrade (option B in the task brief: `font-size: 28px` + `transform: scale(0.5)`, which reopens vertical-align and layout-box compensation). Tests in `renderMarkdown.test.ts` assert the SVG uses `ex` units (documents the root cause), that `MATHJAX_CONTAINER_CSS` sets `font-family` + `font-size`, and that the rule sets `shape-rendering` + `text-rendering` to `geometricPrecision`.

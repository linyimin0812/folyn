...
> Executable contracts for internationalizing Quill desktop UI. zh/en supported.
...
`apps/desktop/src/store/localeStore.ts` — zustand store, OWN localStorage key `quill:locale` (NOT the centralized `settings:all` blob).
...
```ts
// Correct: dedicated `quill:locale` localStorage key, sync-readable.
...
| Locale switch live-persists | `src/i18n/localeStore.test.ts` | After `setLocale('en')`: `i18n.language === 'en'` AND `localStorage.getItem('quill:locale') === 'en'` |

...
//! installed plugins.
//!
//! Sandbox-tier plugins live under `~/.folyn/plugins/<id>/`. The URI scheme
...
//! helpers, the `folyn-plugin://` URI parser + CSP, and the
...
/// Parse a `folyn-plugin://localhost/<id>/<path>` URI into `(plugin_id, file_path)`.
///
...
/// host RPC bridge. `script-src 'unsafe-inline' folyn-plugin:` lets the
...
/// latter. It does not weaken the sandbox model: a plugin's own JS already
...
/// directives, so `eval` grants no capability the plugin did not already
...
pub const PLUGIN_CSP: &str =
    "default-src 'none'; script-src 'unsafe-inline' 'unsafe-eval' folyn-plugin:; style-src 'unsafe-inline' folyn-plugin:; connect-src folyn-plugin:; worker-src folyn-plugin: blob:; img-src folyn-plugin: data: blob:; font-src folyn-plugin: data:; media-src folyn-plugin: data: blob:";

/// Resolve `~/.folyn/plugins/` using the Tauri path resolver.
pub fn plugins_dir(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    let home = app.path().home_dir().map_err(|e| e.to_string())?;
    Ok(home.join(".folyn").join("plugins"))
}

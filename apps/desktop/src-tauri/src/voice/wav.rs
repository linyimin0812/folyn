//! WAV encoder for 16 kHz / mono / 16-bit little-endian PCM samples.
//! Ported verbatim from openless `asr/wav.rs` — the SFSpeechURLRecognitionRequest
//! feeds on a complete WAV file, so the Apple Speech provider writes a temp
//! file with this header before recognition.

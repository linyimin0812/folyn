...
describe('sessionStorage', () => {
  ...
  it('writes files under ~/.mochi/vaults/<vaultId>/', async () => {
    await sessionStorage.saveSession(VAULT, 'path-check', { ok: true });
    const expected = '/mock/home/.mochi/vaults/vault-1/path-check.json';
  ...
  it('returns null on corrupted JSON instead of throwing', async () => {
    ...
    await mkdir('/mock/home/.mochi/vaults/vault-1');
    await writeTextFile('/mock/home/.mochi/vaults/vault-1/broken.json', '{not valid json');
  ...
  it('ensureDir is idempotent — does not re-create existing directory', async () => {
    ...
    expect(await exists('/mock/home/.mochi/vaults/vault-1')).toBe(true);

...
describe('buildCsp', () => {
  it('keeps the Tauri-required baseline', () => {
    ...
    expect(p).toContain("script-src 'self' 'unsafe-inline' 'unsafe-eval' 'wasm-unsafe-eval' blob: folyn-plugin:");
    expect(p).toContain("img-src 'self' data: blob: asset: http://asset.localhost http://*.clouddn.com https://*.r2.dev https://*.aliyuncs.com");
    expect(p).toContain("connect-src 'self' ipc: http://ipc.localhost folyn-plugin: https://*.qiniup.com https://*.r2.cloudflarestorage.com https://*.aliyuncs.com");
    expect(p).toContain("frame-src 'self' blob: data: folyn-plugin:");
  });

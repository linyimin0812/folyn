...
- `## 笔记` 段是散文式（不托管）：用户自由写，feynman/selftest/sq3r 动作在此段段尾追加 callout 块。
...
- 始终 append-only：feynman/selftest/sq3r 动作只在 `## 笔记` 段段尾追加 callout，不删除或改写已有内容。
- research/plan 动作只输出文本建议行，绝不直接编辑主题文档（落盘由 studyStore 捕获 effect 处理）。

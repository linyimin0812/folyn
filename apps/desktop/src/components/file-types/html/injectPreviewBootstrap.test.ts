...
describe('injectPreviewBootstrap', () => {
  it('#1 injects light-theme style and anchor script into a full document', () => {
    ...
    const doc = parse(out);
    const style = doc.querySelector('style[data-folyn-preview="light"]');
    expect(style).not.toBeNull();
    expect(style?.textContent).toContain('color-scheme:light');
    expect(style?.textContent).toContain('background:#fff');

    const script = doc.querySelector('script[data-folyn-preview="anchors"]');
  ...
  it('#3 normalizes a fragment (no head/body) into a valid document with bootstrap', () => {
    ...
    expect(doc.querySelector('style[data-folyn-preview="light"]')).not.toBeNull();
    expect(doc.querySelector('script[data-folyn-preview="anchors"]')).not.toBeNull();
  ...
  it('#4 does not strip existing user <style> or <script> blocks', () => {
    ...
    const doc = parse(out);
    const userStyle = Array.from(doc.querySelectorAll('style')).find((s) => !s.hasAttribute('data-folyn-preview'));
    expect(userStyle?.textContent).toContain('color:red');
    const userScript = Array.from(doc.querySelectorAll('script')).find((s) => !s.hasAttribute('data-folyn-preview'));
  ...
  it('#5 anchor script implements #hash in-document scroll and non-# dead-link (string contract)', () => {
    ...
    const script = parse(out).querySelector('script[data-folyn-preview="anchors"]')?.textContent ?? '';

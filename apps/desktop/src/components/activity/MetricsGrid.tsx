...
export function MetricsGrid({ cards }: MetricsGridProps) {
  ...
  const renderCard = (c: MetricCard) => {
    ...
    return (
      <div key={c.id} className="relative bg-surf2 border border-brd2 rounded-lg p-3 min-w-[120px]">
        <button
        ...
        <p className="m-0 mb-1 text-[11px] text-t2 pr-5 truncate">{c.label}</p>
    ...
    <div className="grid grid-cols-[repeat(auto-fit,minmax(120px,1fr))] gap-3 mb-5">
      ...
      {rest.length > 0 && (
        <div className="col-span-full">
          ...
          {expanded && (
            <div className="grid grid-cols-[repeat(auto-fit,minmax(120px,1fr))] gap-3 mt-3">
              {rest.map(renderCard)}
            </div>
          )}
        </div>

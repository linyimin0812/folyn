...
export function TerminalPanel({
  ...
  return (
    <div
      ...
      style={{ height }}
    >
      <div className="flex items-center gap-1 h-[34px] shrink-0 px-2 bg-panel border-b border-brd overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {sessions.map((s) => (
          <div
            key={s.id}
            className={`group flex items-center gap-1.5 h-[26px] px-2.5 rounded-[6px] text-[13px] font-mono cursor-pointer whitespace-nowrap shrink-0 transition-colors duration-150 select-none ${
              s.id === activeId
                ? 'bg-surf2 text-t1'
        ...
        <button
          className="shrink-0 w-[28px] h-[26px] flex items-center justify-center rounded-[5px] text-t3 border-none bg-transparent cursor-pointer transition-colors duration-150 hover:bg-hov hover:text-t1"

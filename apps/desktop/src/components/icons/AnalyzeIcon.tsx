...
/** Project-analysis panel icon. Inactive state uses the SVG's native
 ...
 * Wiki panel button. Active state is read from `featurePanelStore.
 * activePanelId` so the stored ReactNode needs no prop wiring — the
 * hook re-renders when the ActivityBar selection flips.
 *
...
export function AnalyzeIcon({ size = 18 }: { size?: number }) {
  const active = useFeaturePanelStore((s) => s.activePanelId === 'analyze');
  const theme = useAppearanceStore((s) => s.theme);
  const dataUri = useMemo(() => {
    if (typeof document === 'undefined') return '';
    const stripped = analyzeSvgText
      .replace(/^<\?xml[^?]*\?>\s*/, '')
      .replace(/^<!DOCTYPE[^>]*>\s*/i, '')
      .trim();

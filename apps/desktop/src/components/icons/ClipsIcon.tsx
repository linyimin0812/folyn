...
/** Clips panel icon. Inactive state uses the SVG's native colors
 * (no substitution). Active state rewrites fill+stroke to the resolved
 * `--acc` so the icon turns blue when selected, matching the Wiki panel
 * button. Active state is read from `featurePanelStore.activePanelId` so
 * the stored ReactNode needs no prop wiring — the hook re-renders when
 * the ActivityBar selection flips.
 *
...
export function ClipsIcon({ size = 18 }: { size?: number }) {
  const active = useFeaturePanelStore((s) => s.activePanelId === 'clips');
  const theme = useAppearanceStore((s) => s.theme);
  const dataUri = useMemo(() => {
    if (typeof document === 'undefined') return '';
    const stripped = clipsSvgText
      .replace(/^<\?xml[^?]*\?>\s*/, '')
      .replace(/^<!DOCTYPE[^>]*>\s*/i, '')
      .trim();

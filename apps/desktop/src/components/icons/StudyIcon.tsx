...
/** Study (学习) page icon. Inactive state uses the SVG's native colors
...
export function StudyIcon({ size = 18, active = false }: { size?: number; active?: boolean }) {
  const theme = useAppearanceStore((s) => s.theme);
  const dataUri = useMemo(() => {
    if (typeof document === 'undefined') return '';
    const stripped = studySvgText
      .replace(/^<\?xml[^?]*\?>\s*/, '')
      .replace(/^<!DOCTYPE[^>]*>\s*/i, '')
      .trim();

    ...
    <div className="dlg-overlay" onClick={onCancel}>
      <div className="dlg" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 380 }}>
        ...
        <div className="dlg-ft">
          <button className="btn btn-g btn-sm" onClick={onCancel}>{t('vault:delete.cancel')}</button>
          <button className="btn btn-sm" onClick={onConfirm} style={{ background: 'var(--danger, #e53935)', color: '#fff' }}>{t('vault:delete.delete')}</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="vault-page flex-1 flex flex-col overflow-hidden bg-surf relative">
      ...
      <div className="ph py-[18px] px-[22px] pb-3.5 border-b border-brd bg-panel shrink-0 flex items-start justify-between gap-3">
        <div>
          <div className="text-[9.5px] font-semibold text-t3 uppercase tracking-[.12em] mb-0.5">{t('vault:page.tagline')}</div>
          <h1 className="text-[19px] font-bold text-t1 tracking-[-0.02em] m-0">{t('vault:page.title')}</h1>
          <p className="text-xs text-t2 mt-[3px]">{t('vault:page.description')}</p>
        </div>
        <div className="flex gap-1.5 shrink-0">
          <button className="btn btn-p btn-sm" onClick={() => setShowCreateDialog(true)}>
            {t('vault:page.newVault')}
          </button>
        </div>
      </div>
      ...
      <div className="py-[18px] px-5 overflow-y-auto flex-1">
        <div className="text-[10px] font-semibold text-t3 uppercase tracking-[.1em] mb-[9px]">{t('vault:page.myVaults')}</div>
        <div className="vc-grid grid grid-cols-[repeat(auto-fill,minmax(210px,1fr))] gap-[11px] mb-5">
              ...
              >
                ...
                <div className="flex gap-3 mt-2 pt-2 border-t border-brd">
                  <div className="text-[10.5px] text-t3">
                    <strong className="text-t2 font-medium">{vault.providerType}</strong> {t('vault:page.typeSuffix')}
                  </div>
                </div>
          ...
          >
            ...
            <div style={{ fontSize: 11 }}>{t('vault:page.newVaultShort')}</div>
          </div>
        </div>

        {currentVault && (
          <>
            <div className="text-[10px] font-semibold text-t3 uppercase tracking-[.1em] mb-[9px]">{t('vault:page.fileBrowse', { name: currentVault.name })}</div>
      ...
      {deleteConfirmId && (
        <ConfirmDialog
          message={t('vault:delete.confirm', { name: vaults.find((v) => v.id === deleteConfirmId)?.name || '' })}

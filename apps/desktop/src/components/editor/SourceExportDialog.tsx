/**
 * Source-file export dialog. Lets the user pick the output target
 * (download locally — default, or upload to a configured storage provider).
 * Replaces the old two-step ExportMenu items ("源文件" download + "分享到云端"
 * upload) for source files with one dialog — mirrors SingleDocExportDialog
 * but drops imageMode (source files have no images to inline/upload).
 */
...
import { exportActiveSource, shareActiveBytesToCloud } from '@/hooks/useExport';
...
export function SourceExportDialog({ docName, onClose }: SourceExportDialogProps): React.JSX.Element {
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      if (output === 'upload') {
        const url = await shareActiveBytesToCloud({ fileProviderId: fileProviderId ?? undefined });
  ...
  }, [output, fileMissing, fileProviderId, t]);

...
export function PetBubbleApp(): JSX.Element {
  ...
  // Fire a jump: emit `pet://bubble-action` to the main window. For app
  // launches we keep the bubble open — if the app isn't whitelisted, the main
  // window emits `pet://bubble-authorize-request` back and the authorize UI
  // must render over the still-visible card (closing here would null `bubble`
  // and the `!bubble` short-circuit hides the authorize prompt — user sees
  // nothing and assumes the click did nothing). URL launches never hit the
  // authorize path, so they close immediately. Success has no return signal,
  // so TTL or the ✕ button closes any remaining case (e.g. whitelisted app).

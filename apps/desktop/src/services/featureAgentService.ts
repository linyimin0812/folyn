...
/** Feature 子目录名前缀/后缀（双下划线包裹，与现有 `__daily__` 约定一致）。 */
 ...
 * - schedule 走 bespoke 流程（getFeatureAgentSendOptions 仅给 adapter.send 提供 options）。
 */
...
export async function seedAgentFiles(manager: VaultManager): Promise<SeedAgentResult[]> {
  ...
  // 写诊断日志到 <vault>/.folyn-tmp/feature-agent-seed.log（失败不抛）。
  await writeSeedLog(manager, results);
  return results;
}

/**
 * 把播种结果写入 `<vault>/.folyn-tmp/feature-agent-seed.log`。
...
/**
 * 给 bespoke feature service（schedule）用的轻量辅助：返回 `adapter.send`

 ...
 * `allow-same-origin`) loading from `quill-plugin://localhost/<id>/<html>`,
...
import type { Plugin, PluginContext, PluginLoader, PluginManifest } from '@quill/plugin-host';
...
function createPluginIframe(manifest: PluginManifest): HTMLIFrameElement {
  ...
  const src = `quill-plugin://localhost/${manifest.id}/${html}`;

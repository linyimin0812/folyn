...
interface ToolWindowState {
  ...
  /** Close all windows for a extension (used on deactivate). */
  closeAllForExtension: (extensionId: string) => Promise<void>;
...
export const useToolWindowStore = create<ToolWindowState>((set, get) => ({
  ...
  open: async (extensionId, tool) => {
    if (!isTauri()) return;
    // ponytail: window creation is routed through the Rust
    // `open_extension_tool_window` command so the fullscreen close handling and
    ...
    // cross-window handle registers an event target on the isolated extension
    ...
    // for isolated extension webviews has no Tauri APIs — so any close/cleanup
    ...
    const title = tool.title ?? `${extensionId}/${tool.id}`;
    let label: string;
    try {
      label = await invoke<string>('open_extension_tool_window', {
        extensionId,
        toolId: tool.id,
        entry: tool.entry,
        title,
  ...
  close: async (label) => {
    ...
    if (existing) {
      await existing.close().catch((err: unknown) => {
        console.warn(`[extension-host] failed to close tool window "${label}":`, err);
      });
    }
    get().remove(label);
  },

  closeAllForExtension: async (extensionId) => {
    const toClose = get()
      .windows.filter((w) => w.extensionId === extensionId)
      .map((w) => w.label);

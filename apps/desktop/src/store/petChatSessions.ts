...
// The main window's AI panel persists its sessions to ~/.folyn/vaults/<vaultId>/
// (vault-scoped, via aiSessionPersistence.ts). The pet-panel chat is its OWN
// conversation surface, so it persists to ~/.folyn/pet-chat/ instead — the two
...
async function getBasePath(): Promise<string> {
  ...
  basePath = await join(home, '.folyn', 'pet-chat');
  return basePath;
}
 ...
 *  ~/.folyn/pet-chat/ with NO vault subdirectory (pet chat is vault-free). */

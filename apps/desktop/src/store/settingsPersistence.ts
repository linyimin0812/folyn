...
export interface PersistSlice {
  /** Filename stem under ~/.folyn/storage/. Must be filename-safe
...
const EXPECTED_SLICES = [
  ...
  'translation', 'activityCollectors',
] as const;
 ...
 *  Provider config lives in ~/.folyn/providers/ — load from disk AFTER the
...
export async function loadSettings(): Promise<Record<string, unknown> | null> {
  ...
  // ponytail: storage provider creds live in ~/.folyn/image-hosts/ — load

...
describe('useAiConfigStore.featureCliAdapter', () => {
  ...
  it('getFeatureAdapter returns the override when set', () => {
    ...
    expect(getFeatureAdapter('other')).toBe('claude');
  });
  ...
  it('getFeatureCliPath resolves per-feature adapter binary path (not global cliPath)', () => {
    ...
    expect(getFeatureCliPath('other')).toBe('/usr/local/bin/claude');
  });
  ...
  it('hydrate applies featureCliAdapter blob (string values only)', () => {
    useAiConfigStore.getState().hydrate({
      featureCliAdapter: { wiki: 'pi', bogus: 42, other: '' },
    });
    ...
    expect(map.other).toBeUndefined(); // empty string dropped
  });
});
...
describe('useAiConfigStore.addSelectedModelId', () => {
  ...
  it('persists to ~/.folyn/providers/settings.json', async () => {
    ...
    const base = await join(await homeDir(), '.folyn', 'providers');
...
describe('useAiConfigStore.removeSelectedModelId', () => {
  ...
  it('persists removal to ~/.folyn/providers/settings.json', async () => {
    const store = useAiConfigStore.getState();
...
describe('useAiConfigStore custom providers', () => {
  ...
  it('addCustomProvider persists the def to ~/.folyn/providers/customer/providers.json', async () => {
    useAiConfigStore.getState().addCustomProvider({
      id: 'disk-test',
      name: 'Disk',
      adapterFamily: 'openai-completions',

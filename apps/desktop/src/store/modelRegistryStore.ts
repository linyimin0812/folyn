 ...
 * `settings:all` blob). No separate `~/.folyn/models.json` file — the PRD
 * named a separate file, but Folyn's existing convention is one aggregated
...
export const useModelRegistryStore = create<ModelRegistryState>((set, get) => ({
    ...
    try {
      ...
      void (async () => {
        try {
          ...
          // (`~/.folyn/providers/provider-models.json`) so ownerMap grows
  ...
  setModelCapabilities: (providerId, modelId, capabilities) => {
    ...
    persist();
    // ponytail: also persist to ~/.folyn/providers/provider-models.json so
